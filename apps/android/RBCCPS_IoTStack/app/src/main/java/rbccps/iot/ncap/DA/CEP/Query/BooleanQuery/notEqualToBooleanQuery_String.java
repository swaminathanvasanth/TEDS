...
public class notEqualToBooleanQuery_String extends BooleanQuery{
	int parameter;
	public notEqualToBooleanQuery_String(String in_query,int queryParameters)
	{
		super(in_query);
	...
	public boolean process(HashMap<String,String> currentDataMap)
	{
		...
		ArrayList<String> parametersStrings=new ArrayList<String>();
		if(parameter==0)
		{		
			parametersStrings.add(currentDataMap.get(parametersNames[0]));
			parametersStrings.add(currentDataMap.get(parametersNames[1]));
		}
		else
		{	
			String a=parametersNames[0];
			parametersStrings.add(currentDataMap.get(a));
			String userGivenValue=parametersNames[1].replace("(","");
			userGivenValue=userGivenValue.replace(")","");
			parametersStrings.add(userGivenValue);
		}
