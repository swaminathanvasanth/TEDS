...
public class CenterPanel extends Composite implements SelectionHandler<Integer>, IFormSelectionListener, SubmitListener, LayoutChangeListener, ICenterPanel{
	...
	private int SELECTED_INDEX_XFORMS_SOURCE = 0;

	/** Index for the design surface tab. */
	private int SELECTED_INDEX_DESIGN_SURFACE = 1;
	
	/** Index for the javascript source tab. */
	private int SELECTED_INDEX_JAVASCRIPT_SOURCE = 2;
	...
	private static boolean showXformsSource = false;
	private static boolean showJavaScriptSource = false;
	private static boolean showLayoutXml = false;
	private static boolean showLanguageXml = false;
	private static boolean showModelXml = false;
	...
	FormRunnerView formRunner = new FormRunnerView();
	...
	// FormDesignerController controller = new FormDesignerController(this, null);

	 /* static String xformxml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
			+ "<xforms xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\">"
			+ "<model>"
			+ "<instance id=\"new_form1\">"
			+ "<new_form1 name=\"New Form1\" id=\"1\" formKey=\"new_form1\">"
			+ "<question1/>"
			+ "</new_form1>"
			+ "</instance>"
			+ "<bind id=\"question1\" nodeset=\"/new_form1/question1\" type=\"xsd:string\"/>"
			+ "</model>" + "<group id=\"1\">" + "<label>Page1</label>"
			+ "<input bind=\"question1\">" + "<label>UUID         - </label>"
			+ "</input>" + "</group>" + "</xforms>";
     */
	static xform form = new xform();
	static String xform = form.getxml(); 
	...
	public void onSelection(SelectionEvent<Integer> event){
		selectedTabIndex = event.getSelectedItem(); // SELECTED_INDEX_PREVIEW
...
*/
	...
	private void initXformsSource(){
		...
		txtXformsSource.setText(xform);
		txtXformsSource.setReadOnly(true);
		FormUtil.maximizeWidget(txtXformsSource);
		// controller.openFormDeffered(1, true);
	}
	...
	private void initPreview(){
		...
		previewView.setHeight("1000"+PurcConstants.UNITS); //707"+PurcConstants.UNITS
	...
	private void setupRunner() {
		...
		DeferredCommand.addCommand(new Command(){
			public void execute() {
				try{
					String xml = getXformsSource().trim();
					if(xml.length() > 0){
						...
						formDef.setReadOnly(false);

						
						if(formDef.getLayoutXml() != null)
							setLayoutXml(formDef.getLayoutXml(), false);
						else{
							...
							formDef.setXformXml(getXformsSource());
							formDef.setLayoutXml(getLayoutXml());
						}

						if(formDef.getJavaScriptSource() != null)
							setJavaScriptSource(formDef.getJavaScriptSource());
						else
							formDef.setJavaScriptSource(getJavaScriptSource());
						...
						if(locales != null){
							...
							setLanguageXml(formDef.getLanguageXml(), false);
						}

						loadForm(formDef,formDef.getLayoutXml());
						format();
					}
					FormUtil.dlg.hide();
				}
